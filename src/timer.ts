...
var vue3Timer = {
  ...
  setInterval: function (cb: any, interval: number, context= undefined, oneTime=false) {
    this.id++
    var id = this.id
  ...
  onMessage: function (e: any) {
    switch (e.data.message) {
      case 'interval:tick':
        ...
        if (e.data.message.oneTime) worker.postMessage({ command: 'interval:clear', id: e.data.id })
        break
